...
const fileOffsets = new Map(); // เก็บขนาดไฟล์ล่าสุดเพื่อดึงเฉพาะส่วนที่เขียนเพิ่ม
...
// ฟังก์ชันโหลดข้อมูลประวัติทั้งหมดตั้งต้น
...
function setupDirectoryWatchers() {
  ...
  if (fs.existsSync(CODEX_DIR)) {
    try {
      ...
      console.log('Codex watch notice:', e.message);
    }
  }
...
const server = http.createServer((req, res) => {
  ...
  // 1. Endpoint: GET /api/tokens/history
  ...
  // 2. Endpoint: GET /api/tokens/live (Server-Sent Events)
